...
import { getClarityDaily, getClarityPages, getClient } from "@/lib/data";
    ...
    return (
      <>
        ...
        <section className="view">
          <ConnectState kind="clarity" client={client} />
        </section>
      </>
    );
  }
    ...
    <>
      ...
      <section className="view">
        <div className="view-actions">
          ...
          <RefreshClarity clientId={client.id} />
        </div>

        {secrets?.sync_error && <p className="form-error mb-4">{secrets.sync_error}</p>}
        ...
        {!daily.length ? (
          <Card title="Sin datos todavía">
            <EmptyState label="Esperando la primera foto">
              ...
              <Link href={`/clientes/${client.slug}/web/conectar`} className="link-connect">
                Revisar la conexión
              </Link>
            </EmptyState>
          </Card>
          ...
          <>
            ...
            <div className="grid g-2-1 mb-4">
              <Card title="Páginas más vistas" hint={pages.length ? `${pages.length} páginas` : undefined}>
                {pages.length ? (
              ...
              <Card title="Dispositivos" hint={devices.size ? undefined : "Sin datos"}>
                {devices.size ? (
                  <StageBars
